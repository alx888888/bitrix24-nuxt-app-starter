...
Create a new Bitrix24 local server app starter in an empty folder using a script-first workflow. The script copies a `platform-only` template, applies placeholders, creates canonical agent rules (`.agents/rules`) plus compatibility mirrors (`.qoder/.codex/.antigravity`), validates the generated contract, and leaves a clean baseline for later capability work.
...
7. Give short next actions: `npm install`, `npm test`, `npm run typecheck`, `npm run build`, `npm run dev`, deploy, Neon, Bitrix24 install.
...
- Default rule targets: `agents,qoder,codex,antigravity`.
- `.agents/rules` stays canonical; mirrors are compatibility copies.
...
- B24UI component guide (large): `assets/template/docs/reference/b24ui-llms-full.txt`

For B24UI lookup, prefer grep:

```bash
rg -n "B24(Button|Badge|Table|Input|FormField)" assets/template/docs/reference/b24ui-llms-full.txt
```
...
4. In the fresh project run `npm install`, `npm test`, `npm run typecheck`, `npm run build`, `npm run lint`.
