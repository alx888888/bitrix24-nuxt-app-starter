...
- Direct server imports из `h3` требуют прямую dependency `h3@^1.15`, без перехода на `h3@2` release candidate.
...
- dead-code audit command `npm run audit:dead-code`
- production smoke command `npm run smoke:production -- --base-url https://<domain>`
...
- При добавлении иконок используется только official `b24icons`.
...
- Для Bitrix24 activity skeleton допустимо выполнить `npm run capability:create -- <capability-name> --kind bizproc-activity`.
...
- Runtime logging не сохраняет raw request/result payload по умолчанию.
- Tokens, secrets, application_token, access_token, refresh_token и raw exception objects не пишутся в logs.
- Diagnostic endpoints показывают короткие status/result поля; подробные причины идут через controlled error contracts.
...
3. Для deployed app: `npm run smoke:production -- --base-url https://<domain>`
