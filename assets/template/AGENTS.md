...
2. `docs/reference/bitrix24_dev_resources.md`
3. `docs/reference/official-stack-map.md`
4. релевантные файлы в `.agents/rules/` и совместимых зеркалах
5. `docs/architecture/api-contracts.md`, если change set затрагивает API или types
...
- Change set без обновленного теста, `npm test`, `npm run typecheck`, `npm run build` не закрывать. Если проверка недоступна, это фиксировать в отчете.
